...
def search_shortest_youtube_pose(word):
    ...
    channel_id = get_channel_id_by_name("Goa Board of Education")
    if not channel_id:
        print("❌ Channel not found.")
        return None

    # Search videos within this specific channel
    search_request = youtube.search().list(
        q=query, part="id,snippet", type="video", maxResults=10, channelId=channel_id
    )
    search_response = search_request.execute()
    ...
    for item in details_response["items"]:
        ...
        if duration <= 20 and duration < min_duration:
