...
def concatenate_poses_simple(poses: List[Pose], padding: NumPyPoseBody, interpolation='linear') -> Pose:
...
def find_best_connection_point(pose1: Pose, pose2: Pose, window=0.3):
    ...
    if p1_size == 0 or p2_size == 0:
        # If either is 0, simply return the start of the second pose
        # This avoids the reshape error and connects the poses directly
        return len(pose1.body.data), 0
...
def smooth_concatenate_poses(poses: List[Pose], padding=0.20) -> Pose:
    ...
    print('Concatenating...')
    single_pose = concatenate_poses_simple(poses,padding_pose)
    print('Smoothing...')
